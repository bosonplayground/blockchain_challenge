...
contract BosonEscrow is Ownable {
    ...
    function placePayment(address buyer, uint256 amount) external onlyOwner {
        require(
            ...
            "BosonEscrow: token transfer failed"
        );
        accountsSet.add(buyer);
        escrowBalances[buyer] = escrowBalances[buyer] + amount;
    }
    ...
    function refund(address buyer, uint256 amount) external onlyOwner {
        require(
            ...
            "BosonEscrow: escrow balance too low"
        );
        require(
    ...
    function pay(
        ...
        require(
            IERC20(tokenAddress).transfer(seller, amount),
            "BosonEscrow: token transfer failed"
        );
        escrowBalances[buyer] = escrowBalances[buyer] - amount;
    }
