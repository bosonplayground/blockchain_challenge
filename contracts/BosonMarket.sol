...
contract BosonMarket is Ownable {
    ...
    enum eOrderStatus { Ordered, Completed, Complained }
    struct ItemData {
        bytes32 title;
        uint256 price;
    ...
    event OnNewItem(uint256 indexed itemId, address indexed seller, bytes32 title, uint256 price, uint32 quantity);
    ...
    * @dev Check the caller has created the order referenced with the given orderId
    ...
    modifier onlyOrderBuyer(uint256 orderId) {
        ...
        require(msg.sender == orders[orderId].buyer, "BosonMarket: NOT AUTHORIZED");
        _;
    }
    ...
    * @dev Check the caller has created the item referenced with the given itemId
    ...
    modifier onlyItemSeller(uint256 itemId) {
        ...
        require(msg.sender == itemDatas[itemId].seller, "BosonMarket: NOT AUTHORIZED");
        _;
    }
    ...
    * @dev Check the order referenced with the given orderId is in the given status
    ...
    modifier orderStatus(uint256 orderId, eOrderStatus status) {
        require(isValidOrderId(orderId), "BosonMarket: Invalid orderId");
        require(orders[orderId].status == status, "BosonMarket: Invalid Order State");
        _;
    }
    ...
    * @dev Order a set of items, given a quantity for each of them
     ...
     */
    function order(uint256[] calldata itemIds, uint32[] calldata quantities) external {
        require(itemIds.length == quantities.length, "BosonMarket: NON CONSISTENT ARGUMENTS LENGTH");
        ...
        for(uint32 i = 0; i < itemIds.length; i++) {
            ...
            require(item.availableQuantity >= quantity, "BosonMarket: Requested Quantity Not Available");
            item.availableQuantity -= quantity;
            item.orderedQuantity += quantity;
            totalPrice = totalPrice.add(item.price.mul(quantity));
        }
    ...
    * @dev Complete the order referenced by the given orderId
     ...
     */
    function complete(uint256 orderId) external onlyOrderBuyer(orderId) orderStatus(orderId, eOrderStatus.Ordered) {
        ...
        for(uint32 i = 0; i < _order.itemIds.length; i++) {
            ...
            BosonEscrow(escrowAddress).pay(buyer, seller, item.price.mul(quantity));
        }
    ...
    * @dev Complain for the order referenced by the given orderId
     ...
     * - the caller is the same account that has created this order
     */
    function complain(uint256 orderId) external onlyOrderBuyer(orderId) orderStatus(orderId, eOrderStatus.Ordered) {
        OrderData storage _order = orders[orderId];
        address buyer = _order.buyer;
        for(uint32 i = 0; i < _order.itemIds.length; i++) {
            uint256 itemId = _order.itemIds[i];
            uint32 quantity = _order.quantities[i];
            ItemData storage item = itemDatas[itemId];
            BosonEscrow(escrowAddress).refund(buyer, item.price.mul(quantity));
        }
    ...
    * @dev Gets the requested price for ordering the specified set of items, given a quantity for each of them
     *
     */
    function computePrice(uint256[] calldata itemIds, uint32[] calldata quantities) external view returns (uint256 totalPrice) {
        require(itemIds.length == quantities.length, "BosonMarket: NON CONSISTENT ARGUMENTS LENGTH");
        totalPrice = 0;
        for(uint32 i = 0; i < itemIds.length; i++) {
            uint256 itemId = itemIds[i];
    ...
    * @dev Gets the data of the order referenced by the given orderId
     ...
     */
    function getOrderData(uint256 orderId) external view returns (eOrderStatus status, address buyer, uint256[] memory itemIds, uint32[] memory quantities) {
        require(isValidOrderId(orderId), "BosonMarket: Invalid orderId");
        OrderData storage _order = orders[orderId];
        require((msg.sender == owner()) || (_order.buyer == msg.sender), "BosonMarket: Not Authorized");
